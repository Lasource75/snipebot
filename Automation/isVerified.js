    ...
    const intervalEvent = setInterval(() => {
        ...
        checkNotRug(contractAddress).then((jsonResponse) => {
            ...
            if (jsonResponse.status == '1') { // Le contract est vérifié
                ...
                contract = new web3.eth.Contract(JSON.parse(jsonResponse.result))
                //console.log(contract.methods);

                verifiedContract++;
                fs.appendFile("./logs/verified_contracts.txt", "https://snowtrace.io/address/" + contractAddress + "\n", (err) => {
                    if (err) {
                        console.log("could not write to ./logs/verified_contracts.txt")
                    }
                })
            } else {
